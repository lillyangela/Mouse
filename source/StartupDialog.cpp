...
if (uMsg==WM_COMMAND)
	...
	switch (LOWORD(wparam))
		{
		case IDCANCEL: // Не случилось. [Аэро]Мышь (Did not happen. [Aero] Mouse)
			KillTimer(hdwnd,2);
			GetSettings(hdwnd);
			EndDialog(hdwnd,2);
			return 1;

		case IDOK: 	//Хорошо! Tobii! (Okay! Tobii!)
			KillTimer(hdwnd,2);
			GetSettings(hdwnd);
			EndDialog(hdwnd,0);
			return 1;

		case IDOK2: 	//Хорошо! TheEyeTribe! (Okay! TheEyeTribe!)
			KillTimer(hdwnd,2);
			GetSettings(hdwnd);
			EndDialog(hdwnd,1);
			return 1;
	...
	if (uMsg==WM_INITDIALOG)
	{
		...
		SetWindowText(hdwnd,Internat::Message(8,L"Что используем?"));   //What do we use?
