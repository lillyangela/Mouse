...
void BKBMagnifyWnd::Init()
{
    ...
    //0,
	NULL, // Чтобы в таскбаре и при альт-табе не появлялись лишние окна
	0, BKBInst, 0L );
...
//=========================================================================================
// Если окно невидимо, то показать его. Если видимо, то уточнить координаты точки. (If the window is not visible, then show it. If perhaps, specify the coordinates of the point.)
// Возвращает true, если мы попали в окно, и координаты точки уточнены засчёт увеличения (Returns true, if we got to the window, and the coordinates of specified expense of increasing)
//=========================================================================================
bool BKBMagnifyWnd::FixPoint(POINT *pnt)
{
	if(mgf_visible) // Увеличительное окно открыто (Magnifying window open)
	{
		// прячем окно в любом случае
		//mgf_visible=false;
		//ShowWindow(Mghwnd,SW_HIDE);

		// а попали ли мы в открытое окно? (and got me through the open window?)
		POINT local_point=*pnt;
		// Для отладки (for debugging)
		POINT p1,p2;
		//POINT p3;
		p1=local_point;
		ScreenToClient(Mghwnd,&local_point); // Координаты в клиентском окне (Coordinates in the client window)
		p2=local_point;

		// прячем окно в любом случае (hide window anyway)
		mgf_visible=false;
		ShowWindow(Mghwnd,SW_HIDE);
		...
		{
			// Уточняем координаты для клика (We specify the coordinates for the clique)
			// Насколько точка удалена от середины - делим на увеличение и добавляем к midpoint
			// (As far removed from the middle point - to increase share and add to the midpoint)
	...
	else // окно невидимо, нужно его открыть (window is invisible, it is necessary to open)
	{
...
*/		
		...
		// В этом месте нужно убрать окно с псевдокурсором (At this point, you need to remove the box psevdokursorom)
		BKBTranspWnd::Hide();

		HDC ScreenDC=GetDC(NULL); // Получаем DC экрана (Obtain DC Screen)
		HDC MagBmpDC=CreateCompatibleDC(ScreenDC); // Создаём совместимый DC (Create a compatible DC)
		HBITMAP MagBmp=CreateCompatibleBitmap(ScreenDC,MAGNIFY_WINDOW_SIZE,MAGNIFY_WINDOW_SIZE); // Создаем в нём битмап нужного размера (Create a bitmap in it the right size)
		HGDIOBJ OldBmp=SelectObject(MagBmpDC,MagBmp); // Выбираем битмап в DC (но сохраняем старый!!!) (Choosing a bitmap in DC (but keep the old !!!))
	...
	/*	StretchBlt(MagBmpDC,0,0,MAGNIFY_WINDOW_SIZE,MAGNIFY_WINDOW_SIZE,ScreenDC,
		...
		// делаем окно видимым (теперь это в самом конце) (Make the window visible (it is now at the very end))
		mgf_visible=true;
		ShowWindow(Mghwnd,SW_SHOWNORMAL);

		// Возвращаем окно с псевдокурсором (Return the window psevdokursorom)
		BKBTranspWnd::Show();
